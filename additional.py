...
def first_unassigned_variable(assignment, csp):
    ...
    return utils.first([var for var in csp.variables if var not in assignment])


def mrv(assignment, csp):
    """Minimum-remaining-values heuristic."""
    return utils.argmin_random_tie([v for v in csp.variables if v not in assignment],
...
def num_legal_values(csp, var, assignment):
    if csp.curr_domains:
        ...
        return utils.count(csp.nconflicts(var, val, assignment) == 0 for val in csp.domains[var])
...
solve_usa = copy.deepcopy(usa_csp)
solve_aus = copy.deepcopy(australia_csp)
solve_fra = copy.deepcopy(france_csp)
print("USA - backtrack")
print(backtracking_search(solve_usa))
print("AUS - backtrack")
print(backtracking_search(solve_aus))
print("FRA - backtrack")
print(backtracking_search(solve_fra))
...
# plot_NQueen giving errors so made a new print funcion
NQueensCSP.print_NQueens(solution)
